...
/**
 * Converted from resources/views/korso/onlinemarketing_items/index.blade.php
 * (OnlinemarketingItemController@index) - the "Onlinemarketing Optionen"
 * item-management sub-page linked from Korso/Dashboard.vue's Korso_Admin nav.
 * OnlinemarketingItem::all() with no pagination, matching the old page's
 * plain @forelse loop.
 *
...
defineOptions({
    layout: (h_: typeof h, page: unknown) => {
        const breadcrumbs: BreadcrumbItem[] = [
            ...
            { title: 'Onlinemarketing Optionen', href: '#' },
        ];
...
function destroy(item: OnlinemarketingItem) {
    if (!confirm('Möchten Sie diese Option wirklich löschen?')) return;

    router.delete(`/onlinemarketing_items/${item.id}`, { preserveScroll: true });
}

function rowActions(item: OnlinemarketingItem): RowAction[] {
    return [
        { icon: Pencil, label: 'Bearbeiten', href: `/onlinemarketing_items/${item.id}/edit` },
        { icon: Trash2, label: 'Löschen', variant: 'destructive', onClick: () => destroy(item) },
    ];
}

const ITEM_COLUMNS: DataTableColumn<OnlinemarketingItem>[] = [
    { key: 'name' },
...
<template>
    <Head title="Onlinemarketing Optionen verwalten" />

    <div class="flex flex-1 flex-col gap-4 p-4">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold">Onlinemarketing Optionen verwalten</h1>
            <Link
                href="/onlinemarketing_items/create"
