...
import { ELEKTRO } from '@/lib/handwerkItems';
import AppLayout from '@/layouts/AppLayout.vue';
import type { Auth, BreadcrumbItem } from '@/types';

// Converted from resources/views/handwerk/new/elektro.blade.php ("Mobiliar -
// Elektro"). Same submit target and item-list shape as Show.vue's ELEKTRO
// group (resources/js/lib/handwerkItems.ts).
...
defineOptions({
    layout: (h_: typeof h, page: unknown) => {
        const breadcrumbs: BreadcrumbItem[] = [
            ...
            { title: 'Mobiliar - Elektro', href: '/elektro' },
        ];
...
function initialItemFields() {
    ...
    for (const item of ELEKTRO.items) {
    ...
    <div class="flex flex-1 flex-col gap-4 p-4">
        <form class="grid gap-4 lg:grid-cols-3" @submit.prevent="submit">
            ...
            <div class="bg-card text-card-foreground flex flex-col gap-4 rounded-xl border p-4 shadow-sm lg:col-span-2">
                ...
                <ItemCheckboxGroup :title="ELEKTRO.title" :items="ELEKTRO.items" :form="form" :collapsible="false" />
